...
const ReviewScreen = ({ 
  ...
  const score = review.score || 7.5;
  const strengths = review.strengths || ['Clear communication', 'Good technical understanding'];
  const improvements = review.improvements || ['Could provide more specific examples', 'Consider mentioning best practices'];
  const feedback = review.feedback || 'Good answer with room for improvement. Your understanding of the concepts is solid, but adding more real-world examples would strengthen your response.';
    ...
    <div className="screen">
      ...
      >
        ...
        >
          ...
          <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-full ${getScoreBgColor(score)} ${getScoreColor(score)}`}>
            {score >= 8 ? <TrendingUp className="w-4 h-4" /> : score >= 6 ? <Star className="w-4 h-4" /> : <Lightbulb className="w-4 h-4" />}
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-purple-700 text-sm">
                Remember that interviewers are looking for problem-solving skills and clear communication. 
                It's okay to take a moment to think before answering. Your confidence grows with practice!
              </p>
            </div>
          </div>
        </motion.div>
